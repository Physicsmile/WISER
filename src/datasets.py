...
class CIRRDataset(Dataset):
   ...
   The dataset can be used in 'relative' or 'classic' mode:
    ...
    def __init__(self, dataset_path: Union[Path, str], split: Literal['train', 'val', 'test'],
        ...
        # Validate inputs
        if split not in ['test1', 'train', 'val', 'val_sub']:
    ...
    def __getitem__(self, index) -> dict:
        try:
            if self.mode == 'relative':
                ...
                if self.split in ['train', 'val', 'val_sub']:
...
class EditedImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_name, ext = os.path.splitext(img_name_ext)
...
class CIRCODataset(Dataset):
    ...
    def __init__(self, dataset_path: Union[str, Path], split: Literal['val', 'test'],
        ...
        if mode not in ['relative', 'classic']:
            raise ValueError("mode should be in ['relative', 'classic']")
        if split not in ['test', 'val', 'test_gt', 'val_sub']:
    ...
    def __getitem__(self, index) -> dict:
        ...
        if self.mode == 'relative':
            ...
            if self.split == 'val' or self.split == 'val_sub' :
